  ...
  try {
    ...
    if (quantity > pSize.quantity) {
      return res.status(400).json({
        errors: [
          {
            msg: `Numbers of items exceeding the stock, Stock Left: ${pSize.quantity}`,
          },
        ],
      });
    }

    // const total = quantity * pSize.productStyle.discountedPrice;
    ...
    let cartItemToSave = await CartItem(cartItem);
    await cartItemToSave.save();

    //check the userid of the cart
    let userCart = await Cart.findOne({ user: req.user._id });
    if (!userCart) {
      return res.status(400).json({
        errors: [{ msg: "Please contact the admin. Some Issue with cart" }],
      });
    }
