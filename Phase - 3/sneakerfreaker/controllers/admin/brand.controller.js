...
const addBrand = async (req, res) => {
  try {
    ...
    res.status(200).json({ info: "Brand Accepted" });
  } catch (error) {
...
const deleteBrand = async (req, res) => {
  try {
    ...
    await fileDelete(brand.imagePublicId); // error fileDelete is not a function, but the file is getting deleted from the database, but as expected it is not getting deleted from cloudinary
